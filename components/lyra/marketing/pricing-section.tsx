...
export default function PricingSection() {
  ...
  function handlePlanCta(planKey: 'STARTER' | 'PRO' | 'AGENCY') {
    setLoading(planKey)
    const dest = `/onboard?plan=${planKey.toLowerCase()}&billing=${billing}`
    window.location.href = user
      ? dest
      : `/auth/login?returnTo=${encodeURIComponent(dest)}`
  }
  ...
  return (
    <section id="pricing" className="py-24 bg-background-secondary px-6">
      <div className="max-w-6xl mx-auto">
        <p className="font-sans text-xs text-text-tertiary uppercase tracking-[0.1em] text-center mb-3">Pricing</p>
        ...
        <div className="flex items-center justify-center gap-1 mb-12">
          <div className="flex items-center gap-1 p-1 rounded-lg bg-background-tertiary border border-background-border">
            <button onClick={() => setBilling('monthly')}
              className={`px-4 py-1.5 rounded-md font-sans text-sm transition-colors duration-150 ${billing === 'monthly' ? 'bg-background-secondary text-text-primary border border-background-border' : 'text-text-tertiary hover:text-text-secondary'}`}>
              Monthly
            </button>
            <button onClick={() => setBilling('annual')}
              className={`flex items-center gap-2 px-4 py-1.5 rounded-md font-sans text-sm transition-colors duration-150 ${billing === 'annual' ? 'bg-background-secondary text-text-primary border border-background-border' : 'text-text-tertiary hover:text-text-secondary'}`}>
              Annual
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          {plans.map((plan) => (
            <div key={plan.key} className={`relative rounded-xl border p-6 flex flex-col ${plan.highlight ? 'border-accent-silver bg-background-primary' : 'border-background-border bg-background-primary'}`}>
              {plan.highlight && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="font-sans text-[10px] text-background-primary bg-accent-platinum px-3 py-1 rounded-full">Most popular</span>
                </div>
              )}
              <div className="mb-4">
                ...
                {plan.annualNote && <p className="font-sans text-[10px] text-text-tertiary mt-1">{plan.annualNote}</p>}
              </div>
              ...
              <button onClick={() => handlePlanCta(plan.key)} disabled={loading === plan.key}
                ...
                {loading === plan.key ? <Loader2 size={14} strokeWidth={1.5} className="animate-spin" /> : null}
                {plan.ctaLabel}
              </button>
            </div>
          ))}
