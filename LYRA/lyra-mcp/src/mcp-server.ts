...
export const TOOL_REGISTRY: Record<string, ToolDefinition> = {
  ...
  get_media_upload_url: {
    description: 'Get a URL to upload media you\'ve created yourself (e.g. rendered locally in your own code-execution environment) directly to LYRA\'s storage -- for media that is NOT already hosted anywhere. Returns uploadUrl, fields, and publicUrl. You must then perform the actual upload yourself: an HTTP POST directly to uploadUrl, as a multipart/form-data request containing every key in fields, followed by the file itself under the field name "file" -- it must be the last field in the form, since S3 ignores anything appended after it. Only once that POST succeeds does publicUrl become a real, usable link -- pass it into draft_post\'s or schedule_post\'s media_urls. If your media is already hosted somewhere (e.g. output from a generation tool that returns a URL), use attach_media instead -- it\'s a single call with no separate upload step. Images up to 50MB, video up to 200MB.',
