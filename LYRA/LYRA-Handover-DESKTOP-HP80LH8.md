...
1. **Webhooks vs. polling for Meta comment ingestion + end-to-end latency.** Directly affects the AI response time promise. Need to know: is comment delivery push (webhook) or pull (polling), and what is the realistic delay between a comment being posted and it being available via the API?
2. **Sandbox / test environment.** Need to validate the full comment read → AI process → write-back flow before going live with production accounts.
...
6. **X/Twitter API tier** (Basic, Pro, or Enterprise) — affects rate limits on read/write operations.
7. **REST vs MCP capability parity** — are all capabilities available via both, or are some REST-only / MCP-only?
