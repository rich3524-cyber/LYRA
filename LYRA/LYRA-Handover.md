...
## Changelog
### 2026-08-05 (later) — LYRA MCP server Phase 2: write tools built, hardened, and code-reviewed — not yet deployed
...
**Not yet deployed.** Task 16 (deployment) needs Richard: apply the `McpAuditLog` SQL migration (`prisma/migrations-sql/2026-08-05-mcp-audit-log.sql`, idempotent, safe to paste into the Supabase SQL Editor) against production, set `REDIS_URL` on the Railway `lyra-mcp` service (same Redis instance the main app's worker fleet already uses), then redeploy both services (Railway auto-deploys `lyra-mcp` on push; the main app needs a Netlify rebuild trigger). After that, re-verify via MCP Inspector against a disposable test post/comment before dogfooding for real.
