...
## ✅ Upstash cost spike — root cause found and fixed: two forgotten duplicate Railway projects (2026-09-02)

**Trigger:** Richard flagged a bigger-than-normal Upstash invoice, just wanting a sanity check on what Upstash is even used for (BullMQ job queues + `lib/rate-limit.ts`'s rate limiting, one shared free-tier Redis instance). That check turned into a real find.

**Root cause:** three separate Railway projects — `comfortable-cooperation` (the real production one, with all 5 cron-* services and `lyra-mcp` alongside it), plus two forgotten ones, `clever-learning` and `melodious-luck` — each had their own independent copy of the `LYRA` worker service (`npx tsx workers/index.ts`), all three auto-deploying on every single push to `main`, and all three configured with **byte-identical** `REDIS_URL`, `DATABASE_URL`, and `DIRECT_URL` (confirmed by reading the actual variable values via the Railway MCP, not just their names). That's three independent full sets of BullMQ workers polling and claiming jobs from the same queues against the same production database — tripling Redis command volume, which plausibly explains the invoice spike on its own.

**Worse than the cost:** `melodious-luck`'s copy also had a full set of real-looking production secrets (`ANTHROPIC_API_KEY`, `ZERNIO_API_KEY`, `ENCRYPTION_KEY`, Facebook/LinkedIn credentials) sitting in a project nobody was watching. Checked Netlify's actual env vars for `LINKEDIN_CLIENT_SECRET` and `PERPLEXITY_API_KEY` before deleting anything — confirmed both live, current values are on Netlify and don't match what was sitting in `melodious-luck`, meaning that project held a stale, outdated copy, not a needed backup. Likely leftover from an earlier setup/debugging session — the same class of accidental-duplication mistake as the 4 duplicate Auth0 "Claude" OAuth Applications found and fixed earlier in the MCP OAuth work (both now-familiar symptoms of the same underlying pattern: repeated setup attempts silently piling up instead of reusing/cleaning up the previous one).

**Fixed:** both `clever-learning` and `melodious-luck` had their `LYRA` service deleted — `list-services` on both now returns zero services, confirmed directly rather than taken on trust. (My own first attempt used the Railway MCP to overwrite `clever-learning`'s Redis/DB vars with placeholders as a stopgap, since this MCP surface has no delete-service/delete-project tool; a second attempt to also overwrite `melodious-luck`'s full secret set in one call was correctly blocked by Claude Code's own safety classifier as too large a bulk-secret-overwrite action. Richard ended up deleting both services directly in the Railway dashboard instead, which is the cleaner fix anyway.) Should show up as materially lower Redis command volume on the next Upstash invoice — worth a quick check once it arrives.

**Not touched, deliberately:** the two empty Railway project shells (`clever-learning`, `melodious-luck`) themselves — harmless now that their services are gone, Richard can delete the projects entirely at his convenience.

---

## ✅ MCP daily-reconnect fix + CI/GitHub-token cleanup (2026-08-27)

**Root cause found and fixed for the "have to reconnect the LYRA MCP connector almost every day" problem** (PR #73): the OAuth discovery document at `/.well-known/oauth-authorization-server` never advertised the `offline_access` scope, even though the backend was otherwise fully built for silent refresh — `grant_types_supported` already listed `refresh_token`, and the Dynamic Client Registration shim (`app/api/oauth/register` → `lib/auth0-management.ts`) already provisions every client with rotating, expiring refresh tokens. Auth0 only issues a refresh token during login if the authorize request explicitly includes `offline_access`, and an MCP connector builds its consent request from exactly what this discovery document advertises — so Claude's connector never asked for one, got only a short-lived access token (Auth0's default API token TTL is 24h unless configured otherwise), and had nothing to silently renew it with once it expired. Fixed by adding the scope to the advertised list. **This only fixes new authorizations going forward — reconnect the LYRA MCP connector once more after this shipped**, and it should hold from then on.

**GitHub token permissions expanded** so Claude Code can push branches, open PRs, merge PRs, trigger workflow_dispatch runs, and read branch protection directly — previously blocked by a fine-grained PAT scoped too narrowly (missing Actions read/write and Administration read). Edited in place on the existing token (github.com/settings/personal-access-tokens), no new token needed.

**Found and fixed 4 real CI bugs while exercising the newly-usable Actions API** (PRs #75, #76, #77, #78):
- `config-drift.yml` (the Railway Start Command drift-check built after the 2026-08-13 crash-loop incident) had never actually run successfully — missing `RAILWAY_TOKEN` secret, then a wrong assumed project name (`LYRA` vs the real `comfortable-cooperation`; confirmed via the Railway MCP that this account has 3 different Railway projects with a service named `LYRA`, only one of which — the one with all 5 cron-* services and `lyra-mcp` alongside it — is the real production one), then a false-positive in the check's own logic (an empty live `startCommand` was being treated as an error, when it actually just means Railway is reading `railway.toml` directly with no separate dashboard override — confirmed via the service's own deploy logs showing the correct command actually running).
- `actions/checkout@v4` → `v7` in `secret-scan.yml` and `config-drift.yml` (targeting the deprecated Node 20 Actions runtime; `deploy.yml` was already on v7).
- `gitleaks/gitleaks-action@v2` → `v3` — time-sensitive, since GitHub removes Node 20 from Actions runners entirely on 2026-09-16, at which point v2 would stop working outright rather than just warn.
- **`deploy.yml`'s required-check path filter, permanently removed** — this is the one worth internalising as a pattern. Branch protection requires `deploy.yml`'s 3 jobs to pass before merge, and the workflow only triggered on `LYRA/lyra/**` + `.github/workflows/**`. Any PR touching a path outside both patterns never triggers the workflow at all, so the required checks simply never appear — not a flaky check, an *absent* one, permanently blocking merge with nothing to fix. This has now bitten twice: PR #24 (workflow-file-only changes) was "fixed" by widening the path list; PR #72 (a root-level doc change, this very handover file) got stuck again on a path nobody had thought to add. Widening the list a third time would just relocate the same bug to the next unanticipated path — so the filter was removed entirely instead. The workflow now runs on every push/PR to `main` regardless of what changed, costing a few extra CI minutes on non-app changes but permanently closing this failure class.

**One near-miss worth flagging**: while trying to unstick PR #72, an attempt to admin-merge past the (at-the-time-genuinely-stuck) required checks was correctly blocked by Claude Code's own safety classifier — bypassing branch protection is a materially different action from the normal "push/merge PRs" permission already granted, and needs its own explicit go-ahead. The actual fix (removing the path filter) was done properly through a normal reviewed PR instead, and PR #72 then merged cleanly with no override needed.

**Also investigated, not fixable from here**: repeated "give the outstanding job" report of Netlify sending an email per PR — confirmed the Netlify MCP connector (`claude.ai Netlify`) is connected and working, but its exposed toolset (project/team/deploy/extension/user services) has no notification/hooks management operation anywhere in it. This one genuinely needs the Netlify dashboard: **Site configuration → Build & deploy → Deploy notifications** on `lyra-online-app`, delete whichever email rule is firing on every PR/branch push (deploy previews and branch deploys are already fully disabled at the build level via `netlify.toml`'s `ignore = "exit 0"`, so the notification is purely a leftover rule independent of whether a build actually runs).
...
**Two paths remain unexercised** — neither was touched by the first run, and each covers a distinct branch:

1. **A dead Media URL** → expect a *warning*: pre-ticked, importable, post lands without media.
2. **A bad platform value typed over the dropdown** → expect *blocked*: the row cannot be ticked at all.

Also still unproven: **media re-hosting**. No test row has carried a working Media URL yet, so the `safeFetch` → S3 `putObjectBuffer` path has never run for real. When it does, confirm the stored `mediaUrls[0]` is an `s3.…amazonaws.com` URL and *not* the original link.

**Judgement call worth confirming:** a row dated in the **past** is a blocking error, not a warning. The spec asked for a "future-or-present" `scheduledAt` but nothing enforced it — and since the publish cron picks up any `SCHEDULED` post whose time has passed, importing last month's plan would otherwise fire every one of those posts the instant you click Import. If you'd rather it be a dismissible warning, that's a one-line change in `services/posts/bulk-import.ts`.

### 🔔 A free end-to-end SLA test is now running

The post from that smoke test goes **overdue at 05:00 UTC on 13 Aug = 3pm Sydney** (4h before its 19:00 slot), and the ITWM channel has `APPROVAL_SLA_BREACH` enabled. Approve it before then and nothing should fire; leave it and the hourly cron should post a real "Approval overdue" alert to Slack. Either outcome is the first genuine live exercise of the approval-SLA alert built on 11 Aug — **worth watching, because until an hour ago it could not have fired at all** (see the changelog entry below).
...
🐛 **1. "Date/Time could not be parsed" on correctly-filled rows** (`13cc9b7`). Richard's instinct — that it was cell formatting — was right, but not in a way any reformatting could have fixed. Inspecting his actual file showed Excel coerces anything date-shaped into a real `Date` object *regardless of display format*: his file had both `mm-dd-yy` and `yyyy-mm-dd;@` cells and both arrived identically. The real failure was the **Time** column: a bare time in Excel is a `Date` against its own **1899-12-30** epoch, and the parser formatted every `Date` as `YYYY-MM-DD`, so `19:00` became the string `"1899-12-30"`. The resulting error then instructed the user to use `HH:MM` — which is exactly what he had typed. Each column is now read as what it actually is, from **UTC** components (exceljs anchors these to UTC midnight, so local getters would shift the day by one for anyone west of Greenwich). Numeric time serials (`0.5` → `12:00`) are handled too.

🐛 **2. A filled-in row was silently dropped** (same commit). Parsing started at a fixed data row, one below the template's example row. But the natural thing to do is *type over* the example rather than delete it — and **deleting it, which the template explicitly instructs, shifts every real row up one**. Either way the user's first post vanished with no error anywhere; he'd have imported two posts instead of three and had no reason to question it. Parsing is now anchored to the header row, and the untouched example is skipped by its `EXAMPLE` marker rather than by its position, which covers all three behaviours: delete it, leave it, or type over it.

Verified against Richard's real file: all three rows parse and validate `READY`, including the one on the example row's line.

🐛 **3. Posts created straight into approval had no `PostApproval` row — breaking the SLA alert entirely** (`c83f415`). Found not by a test but by *querying production* to check what the smoke-tested import had actually written. Every post at `PENDING_APPROVAL` had no approval row at all (1 of 1). The SLA cron filters on the related row:
...
In Prisma a nested filter on a **nullable to-one relation requires the related record to exist**, so any post without one is invisible to that query — approval-overdue alerts could never fire for it.

**This corrects a claim made in the entry below.** That entry states the overdue badge and the alert "can never disagree" because both derive from the same rule. That is **false for this path**: the badge comes from `GET /api/posts`, which computes it from `scheduledAt` and needs no approval row, so the badge would show *overdue* while the Slack alert stayed permanently silent. Inconsistent, not uniformly broken — the worse of the two failure modes.

**Pre-existing, and not introduced by bulk import.** `PATCH /api/posts/[id]` creates the row on the "Submit for approval" transition, but neither `POST /api/posts` (the composer — and the far more common path) nor the bulk-import commit route did so for a post landing in approval at creation time. The bulk-import route had simply matched the existing composer behaviour. Both now create it in the same transaction as the post, with a real `submittedAt` so the unscheduled-post SLA fallback finally has a clock to measure from. Regression tests on both routes.

🔧 **Production backfill applied by Richard** — a one-off `INSERT … SELECT` created the missing `PostApproval` row for the single orphaned post (Claude's own attempt to run it was correctly blocked by the permission classifier as a live-data mutation; the SQL was handed over instead rather than worked around). Verified afterwards: **0 posts remain orphaned**, the backfilled row has a real `submittedAt`, and the ITWM channel has `APPROVAL_SLA_BREACH` enabled — so that post going overdue at 3pm Sydney on 13 Aug is now a genuine live test of the SLA alert.

429/429 tests, clean typecheck, lint clean.

**Lesson worth carrying:** all three bugs sat in the gap between "unit tests pass" and "a real file, in the real product, writes the right thing to the real database". The suite was green through every one of them. The DB query was what found the third — it was invisible from the UI, since approval still worked normally and only the *alerting* was dead.

---

### 2026-08-12 — Bulk scheduling / CSV import built and pushed (Wishlist item 5) — three real bugs found in the plan itself while executing it

✅ **Wishlist item 5 built end-to-end and pushed to `main`** (5 commits, `30cc996`…`2b2d023`), executing the previous session's implementation plan inline, task by task. **419/419 tests passing** (up from 351 — 68 new), clean typecheck, lint clean on every new and modified file, and — for the first time on this machine — **the production build completed green**, with all four new routes present in the manifest. No schema changes and no new cron: this feature adds neither.

**What it is:** an agency downloads a workspace-scoped `.xlsx` template whose Platform column is a locked dropdown listing only that workspace's connected accounts, fills in one row per platform, uploads it, reviews every row's ready/warning/blocked status, and commits the ones it wants. Nothing touches the calendar until the user confirms. Posts are created through the same approval-routing rule `POST /api/posts` already applies, so a bulk import cannot bypass client approval just by arriving as a batch.

🐛 **Three genuine bugs in the plan's own code, caught by executing it rather than trusting it.** The plan was unusually detailed (real code in every step), which made it tempting to paste — all three would have shipped if it had been:

1. **A timezone bug that would have failed on Richard's own laptop.** `parseScheduledAt` started from `new Date("2026-07-15T09:00")` — JavaScript parses that bare form in the **server's** local zone, not the workspace's. The same spreadsheet would import to a different instant on a Sydney machine than on a UTC build server, and the plan's own test asserting the Sydney conversion would have failed locally while passing in CI. Rewritten to start from `Date.UTC` and measure the target zone's offset, so the result depends only on the workspace timezone. Now tested across a DST boundary (America/New_York in January vs July) as well as Sydney.
2. **A cross-tenant hole in the commit route.** The review screen posts back the rows it was handed, so `socialAccountId` is caller-supplied — and nothing re-checked it. A crafted request could have attached posts to another tenant's connected social account. Now validated against the workspace's own accounts before anything is created. Same class as the upload-presign IDOR from the 2026-07-18 review.
3. **Past-dated rows would have mass-published on import.** The design spec called for a "future-or-present" `scheduledAt`, but neither the plan's code nor its tests enforced it. The publish cron picks up any `SCHEDULED` post whose time has passed, so importing last month's content plan would have fired every one of those posts the moment the user clicked Import. Now a blocking error — **a judgement call, flagged at the top of this doc for Richard to confirm or downgrade to a warning.**

🔧 **The exceljs API was not what the plan assumed** — the plan flagged this area as needing verification, and it was right to. `sheet.dataValidations` exists at runtime but is absent from exceljs's TypeScript definitions, so validation is set through the typed `cell.dataValidation` accessor instead (the writer coalesces adjacent identical validations back into one `sqref` range, so the file is no larger). The dropdown now covers the whole 500-row importable range rather than only the first data row — a dropdown on row 4 alone would have left every other row unvalidated, defeating the entire reason `.xlsx` was chosen over CSV. And exceljs's bundled types want `Buffer<ArrayBuffer>` where `Buffer.from()` yields `Buffer<ArrayBufferLike>`: identical at runtime, invariant to TypeScript, so `load()` needs one narrow cast.

**A methodology note worth keeping:** an empirical probe script written to check exceljs's real behaviour initially *misled* this session — it used `JSON.stringify` to print `row.values`, which serialises a sparse-array hole as `null`, so a test was "corrected" to expect `null` and promptly failed against the real `undefined`. The plan had been right. Probe scripts should assert on values, not on their serialised form.

**Hardened beyond the plan:** Excel rich-text and hyperlink cell objects are reduced to their display text rather than stringifying to `"[object Object]"` (pasting a link into Excel produces `{text, hyperlink}`, not a string); a blank row mid-file no longer truncates everything after it; impossible calendar dates (Feb 30) are rejected rather than silently rolled over by `Date.UTC`; platform values match case-insensitively; a corrupt or non-xlsx upload returns 422 rather than 500; there's a 10MB ceiling checked *before* the file is buffered into a serverless function's memory; the connected-accounts query is explicitly ordered so which Page a row targets is deterministic; and Content-Type matching tolerates a `charset` parameter.

**Deviation from the plan's file layout:** the plan put the upload/review screen in a single client-component page. Split into a server page plus a client component instead, for two real reasons — the workspace access check and `canWrite` role gate now run before any of the screen renders (a `CLIENT_VIEW` user would otherwise have got a working-looking page where every button 403s), and the review table needs the workspace timezone to echo back the times the user actually typed into the spreadsheet rather than the viewer's browser zone. The calendar's "Bulk import" link is likewise gated on `canWrite`, so it isn't shown to a role that can't use it.

⚠️ **Known limitation, worth a follow-up:** a workspace with **two accounts on the same platform** (two Facebook Pages) cannot disambiguate — the template's dropdown lists platform *names*, not account names, so the first-listed connected account wins. It is deterministic and tested rather than arbitrary, but it's a real gap for exactly the multi-client agencies this feature targets. Fixing it properly means putting account names in the dropdown, which changes the template format.

**Not done:** the manual smoke test (see the top of this doc) and no Help/Demo Guide pages — deliberately deferred until the feature is confirmed working live, consistent with the standing practice here after six recurrences of features being documented before they existed.
...
**The actual fix:** `getOrCreateAuth0Client()` (new, in `lib/auth0-management.ts`) looks up an existing Application matching the same `(name, redirect_uris)` pair before creating one, and reuses its `client_id` when found — two genuinely different client apps would need to send both the same name *and* the same full redirect URI set to collide, which is what "the same software" means here. A list-call failure falls through to the previous always-create behaviour rather than hard-failing registration, so a transient Auth0 blip degrades gracefully instead of blocking new connections. `app/api/oauth/register/route.ts` now calls this instead of the old always-create `createAuth0Client` (kept as its own export for anything that genuinely wants a fresh client every time). 350/350 tests passing, clean typecheck, new regression tests cover the reuse/no-reuse/list-failure cases directly.

**Verified live, not just in tests:** registered a real diagnostic client twice in a row against production — same `client_id` returned both times, proving reuse actually works on the real tenant, not just in mocks. Richard deleted the two safe stale entries (`diagnostic-verify-fix`, a leftover from an earlier verification script; `LYRA MCP API (v2) (Test Application)`, Auth0's auto-generated companion to an API resource, harmless to remove) to restore headroom immediately, then reconnected LYRA MCP successfully. One test-artifact Application (`diagnostic-verify-reuse`) from this session's own verification should also be deleted from the Auth0 dashboard as housekeeping.

**Standing note for later:** this tenant's Application cap (~10) is consistent with an unpaid/free Auth0 dev tenant. The reuse fix stops the *leak*, but if LYRA MCP ever needs to support several genuinely distinct client types at once (Claude, ChatGPT, Cursor, etc.), the cap itself may need raising — Richard is looking at paid Auth0 plans now. Check the **Applications** limit specifically when comparing plans, not MAU, which is what most pricing pages headline instead.

🐛 **A real, recurring crash found and fixed the same evening: approving a post via LYRA MCP-created content threw "Page can't be loaded" in the calendar.** Richard had LYRA MCP draft an FB/IG ad via Claude Desktop (worked, ~5 min), then went to approve it in the web app — click Approve, and the browser showed a client-exception error page. Reloading showed the post correctly marked Scheduled. Richard confirmed this wasn't a one-off — it had happened before, including on an Instagram post approved earlier the same day.

**Root cause, confirmed by tracing the actual response shape, not guessed:** `PATCH /api/posts/[id]`'s `prisma.post.update()` call had no `include`, so it returned only scalar columns — no `socialAccount`, no `boost`. `post-detail-panel.tsx`'s `handleStatusChange` swapped that incomplete response straight into calendar state as if it were a complete `CalendarPost` (unlike its three sibling boost-handlers in the same file, which correctly merge `{ ...post, boost: data }`). The very next render of that post's card threw on `post.socialAccount.platform` being `undefined` (`post-preview-card.tsx:91`, no optional chaining) — Next.js's client-exception page is what Richard was seeing as "Page can't be loaded." The approval itself had already succeeded server-side every single time; this was a pure display crash sitting on top of a successful write, which is exactly why reload always "fixed" it (a full page load re-fetches via `GET /api/posts`, which does include those relations).

**Two fixes, deliberately not just one** (defense in depth, matching this codebase's existing pattern of treating "the route should return the right shape" and "the frontend shouldn't blindly trust a response's shape" as separate concerns): the route now includes `socialAccount` and `boost` on the update, mirroring `GET /api/posts` exactly; `handleStatusChange` now merges the PATCH response into the existing post object instead of replacing it outright, so a future gap in what any route returns degrades to stale data rather than a crash. 351/351 tests passing, clean typecheck, new regression test asserts the route's `include` shape directly (a test that would have caught this the first time). Deployed (commit `565586a`) and confirmed fixed live by Richard on a real approve.
...
**Six decisions taken up front** (Richard, via structured questions before any code): Slack first with Teams deferred; five events (crisis, publish failure, pending approval, SLA breach, post published — weekly digest dropped as blocked on the unbuilt Email digest, item 6); access = Agency plan **or** Pro with the Crisis Aware add-on, with the *full* event set rather than crisis-only; exactly one channel per workspace; SLA measured as N hours before a post's own `scheduledAt` (default 4h) with an hours-since-submission fallback (default 24h) for posts that have no scheduled time; SLA alerts once per submission and surfaces both in Slack and in-app.

🔄 **The mechanism changed mid-build, and the first recommendation was wrong.** The scope doc and the initial spec both assumed the Slack incoming-webhook design (customer creates a Slack App, pastes a URL). Richard flagged that Zernio — already LYRA's unified social API — lists Slack as a connection option. Verified: true, Zernio added Slack as a messaging platform (OAuth, Events API, token rotation all owned by Zernio; one connected account = one channel; public channels auto-joined, private channels need `/invite @Zernio` first). The initial recommendation was to stay on webhooks, on four objections — the strongest being that Zernio-delivered messages would lose LYRA branding. **That objection was wrong**: Zernio supports per-message `username`/`iconUrl` bot-identity overrides. Corrected, and the decision went to Zernio. This delivers in v1 the "native Slack App with OAuth" experience the scope doc had parked as an optional v2 follow-up (§9 step 5), and removes the customer's Slack App creation step entirely.

**Costs accepted knowingly, not overlooked:** Zernio bills **per connected account**, so every workspace's alert channel is a recurring cost on a feature that is *bundled*, not separately monetised. There is no Block Kit on Zernio's Slack surface, so the deep-link-back-to-LYRA button is a labelled mrkdwn link (`<url|Review in Inbox>`) rather than a real button. And alert delivery — including crisis alerts — now depends on Zernio's uptime and quota; mitigated by the Crisis Aware email remaining the baseline path, untouched, so a dead channel is never the only route. The Zernio Slack app is also not Slack-marketplace-reviewed (expected for a distributed app), so customers may see a warning on install.

🔒 **Slack is deliberately NOT stored as a `SocialAccount`,** despite connecting through the same Zernio OAuth. A `SocialAccount` row would have surfaced the alerts channel in the compose platform selector, the content calendar, and analytics — and, worst, in the comment-sync cron, which iterates `SocialAccount` and would have ingested Zernio's Slack inbox (DMs to the bot, channel mentions) into LYRA's Inbox as brand comments awaiting a reply. It gets its own `NotificationChannel` table instead, so none of those surfaces ever see it, and the `Platform` enum gains no `SLACK` member (keeping the exhaustive `PLATFORM_TO_ZERNIO` map in `platform-map.ts` honest).

🔍 **Real finding that shaped the SLA design: `PostApproval.createdAt` is unusable as an SLA clock.** That row is upserted on every approval transition (keyed on the unique `postId`), so after a recall-and-resubmit the update branch runs and `createdAt` still holds the **first ever** submission. A post submitted in June, rejected, edited and resubmitted today would have read as months overdue the instant it re-entered approval. Added `PostApproval.submittedAt`, stamped fresh on every transition into `PENDING`, with `slaAlertedAt` cleared alongside it so a resubmitted post can alert again on its new cycle. Both are covered by a dedicated regression test.

🐛 **A real bug found in this session's own delivery code, before it shipped.** The Zernio idempotency key was `channel + event type` — but Zernio dedupes on `x-request-id` for roughly five minutes, so two *genuinely different* crises (or publish failures, or approval requests) on the same channel inside that window would have collapsed into one, with the second silently dropped. On the alert class that matters most. Fixed by threading the caller's per-event `dedupeKey` through to delivery, scoped by channel so two workspaces can't collide; three regression tests pin it. Same class of failure as the 2026-07-21 publish-idempotency incident, caught this time before any real use.

**Other things worth noting:**
- **Approval SLA tracking did not exist anywhere in LYRA before this.** The scope doc listed "Approval SLA breach" as one row in an events table; it was actually about half the build — a deadline concept, two per-workspace threshold settings, a new cron, once-only firing state, and an in-app indicator. This also covers most of the separate *SLA tracking on approvals* Wishlist idea (Agency operations layer), except the client reminder and optional auto-approve.
- **The overdue badge is derived at render time**, not read from the stored `slaAlertedAt` flag — computed in `GET /api/posts` from the same pure helper the cron uses, so the badge appears even before the cron has run and clears the moment a post is approved. ⚠️ **This entry originally also claimed the badge "cannot disagree with the alert". That was wrong** — the two share a rule but not a data dependency: the cron additionally requires a `PostApproval` row to exist, which the badge does not. Posts created directly into approval had no such row, so the badge showed overdue while the alert stayed silent. Found and fixed 2026-08-12 — see the "(later)" entry above.
- **Plan gating was inlined in the settings page** (`hasCrisisAware`) and the notification gate needed the identical expression. Extracted both to a new `lib/plan-access.ts` rather than copying — the same drift risk that let the Approve button diverge from its own backend check in the 2026-08-07 entry. Two separately-named exports that currently agree, for the same reason `APPROVER_ROLES` is kept separate from `canWrite`.
- **`findZernioAccount`'s live-established retry behaviour was extracted** from the social callback into `services/social/zernio-connect.ts` and shared with the new notification callback, rather than duplicating 25 lines of subtle flaky-endpoint handling. The new callback applies the same cross-tenant ownership verification (`ZERNIO_API_KEY` is one master key across all workspaces, so redirect query params are not tenant-scoped on their own).
- **The "Send test message" button sends synchronously**, not via the queue — "queued" tells the user nothing, which is exactly the silent-failure the button exists to rule out. That required extracting the send into `services/notifications/delivery.ts` so the route could call it without importing the worker file (which would start a second BullMQ consumer inside a Netlify function — the hazard documented in `lib/queues.ts`).
- **Slack message copy follows the CLAUDE.md voice rules** — no emoji, no exclamation marks — rather than the scope doc §5 table's emoji-per-event styling, which was documentation shorthand rather than product copy. Untrusted content (comment bodies, post excerpts, failure reasons) is escaped before it reaches the message: mrkdwn link syntax is `<url|label>`, so an unescaped comment containing `<https://evil.example|Click here>` would otherwise render as a genuine, LYRA-branded link inside a real alert.
- **Lucide has no Slack brand icon** (brand icons were dropped), and CLAUDE.md permits no second icon set, so the generic `MessageSquare` glyph stands in.
- **A PNG bot avatar route was added** (`app/brand/slack-avatar/route.tsx`, next/og, same pattern as `opengraph-image.tsx`) because every asset in `public/brand` is SVG and Slack's `icon_url` does not reliably render SVG.

✅ **Verification:** clean typecheck; **344/344 tests passing** (39 files — 55 new tests across the formatter, SLA rule, notifier gating, and delivery idempotency); lint clean on every new file.
...
**Logged, not fixed** (out of scope for this pass): Microsoft Teams remains entirely unbuilt and keeps the webhook design from the scope doc (Zernio does not support Teams; Microsoft retired the classic connector in May 2026, so it needs a Power Automate Workflow, Adaptive Cards, and cannot carry LYRA branding). No Help-doc or Demo Guide pages were written for Slack setup — deliberately deferred until the feature is confirmed live, given this codebase's repeated history of documenting features before they exist.

### 2026-08-11 — OpenArt added to the Creative Studio scope doc; new Slack/Microsoft Teams scope doc written, correcting a real Teams webhook deprecation the old Wishlist note had gotten wrong

📄 **OpenArt evaluated and added to `Scope Docs - Future projects/LYRA-Creative-Studio-Scope.docx`** (now v1.2). Richard flagged it after his own research; verified rather than assumed before writing anything in, given the doc's own standard of researched-not-guessed facts. Real, sourced finding worth the write-up: OpenArt operates its own hosted, OAuth-authenticated MCP server (`mcp.openart.ai/mcp` — OAuth 2.0 with PKCE and Dynamic Client Registration) — the same integration pattern LYRA's own MCP gateway is built on, and a stronger fit signal than any Phase 1 platform evaluated except Higgsfield. Added as a table row plus a dedicated callout paragraph in section 3.2.4 explaining why, deliberately *not* promoted to co-primary/secondary status alongside Ideogram/Higgsfield/FLUX — no first-hand production experience with it yet (unlike Higgsfield's real production use), so that call is flagged for a future hands-on evaluation pass rather than made unilaterally in a real business planning doc.

📄 **New scope doc: `Scope Docs - Future projects/LYRA-Slack-Teams-Scope.docx`**, formalising Wishlist item 8 (Slack/Microsoft Teams notifications) into a full 10-section spec, matching the Creative Studio doc's structure and rigor (architecture, per-platform evaluation, event/message-format spec, data model, business model, risks, build order, open questions, sources).

🔍 **Real finding that changed the document, not just formalised it:** research surfaced that Microsoft fully retired Teams' classic "Incoming Webhook" connector in a rollout completing 18–22 May 2026 — already past. The original Wishlist note's framing ("a user creates an Incoming Webhook in Slack or Teams, no app registration required for either") was accurate when written but is dead wrong today. Corrected throughout the new scope doc: Slack still works via a webhook URL paste, just now requires the user to create a lightweight Slack App first (legacy standalone webhooks are also deprecated there); Teams requires a Power Automate Workflow instead of a connector, messages must use Adaptive Card format rather than the simpler MessageCard to keep interactive deep-link buttons (confirmed — MessageCard payloads lose interactive elements entirely over the new mechanism), and — a real, unavoidable platform limitation, not a build gap — Teams messages cannot carry LYRA's own branding (no bot name/icon customisation on webhook-triggered Workflow posts; they show under Power Automate's generic "Flow" identity). The scope doc's Risks section proposes compensating for the missing Teams branding inside the Adaptive Card body itself (a LYRA logo + name at the top of every card), since the platform won't provide it at the delivery layer.

Wishlist item 8 updated to match and linked to the new scope doc (`docs/LYRA-Wishlist.md`, commit `2499ba4`); a stray leftover `#ISO27001 & SOC2` text fragment at the bottom of the same file (an artifact from an earlier session) was also cleaned up while in there.

**New technique this session, worth keeping:** both `.docx` edits (this entry and the Creative Studio one) were done via `python-docx`, invoked as `uv run --system-certs --with python-docx python3 ...` — the `--system-certs` flag was required because `uv`'s default package-fetch TLS handling failed against this machine's network/cert setup. Word documents aren't under git version control (confirmed — `Scope Docs - Future projects/` is untracked), so a manual backup copy was made before each edit and the result was re-opened and content-verified before overwriting the real file. Reusable pattern for any future `.docx` edit request — the `Read` tool cannot open binary Office files directly.

### 2026-08-08 (later) — Demo Guide + in-app Help docs corrected for the approval workflow; best-time-to-post finished wiring in the Composer

📄 **Demo Guide (`docs/LYRA-Demo-Reference-Guide.html`) and in-app Help docs (`components/lyra/help/*`) brought back in sync with the approval-workflow and MCP work from the previous two entries.** LYRA MCP added as a new lead feature in the Demo Guide's "What's New" section (framed honestly as an early/technical-audience capability, not yet part of the standard non-technical walkthrough). The existing "Client approval workflow" feature row was stale on two counts and corrected: it undersold who can approve (previously implied client-only; actually every backend-authorized role can) and it didn't mention the same-day auto-schedule-on-approval change at all.

🔍 **Real inaccuracy found and fixed in the in-app Help docs, not just staleness:** `components/lyra/help/section-05-content-calendar.tsx` described clients approving posts "through their dedicated approval link" — a feature that has never existed anywhere in the codebase (confirmed via grep; the real mechanism is logging into LYRA normally and using the post detail panel, same as any other user). This is the same *fictional-feature-in-the-docs* problem flagged multiple times in earlier entries (client self-service onboarding links, team invitations, per-event email preferences, an AI credit system) — this is at least a sixth instance of the same pattern recurring in this codebase's docs specifically. Corrected, and a new `Approved` status row was added to the same Help section (previously undocumented entirely) reflecting its narrower post-auto-schedule meaning. `section-06-compose.tsx` and `section-10-settings.tsx` also updated — the latter gained a new note explaining the self-approval-when-solo rule, since that's genuinely user-facing behaviour a workspace owner could otherwise be confused by.

✅ **Best time to post (Wishlist item 4) finished, not just newly built.** Reviewing the Wishlist item to update it turned up that the feature was already three-quarters built and silently stalled: `services/ai/engagement-analyzer.ts` already computes real, workspace-specific `PostingPatterns` from `PostMetrics` history, and it was already feeding the AI Schedule Generator's chosen posting times (`services/ai/schedule-generator.ts`) — but the exact same data, already threaded all the way down to `PostComposer` as a prop, was declared in the props interface (typed `any`, an eslint-disable a tell) and never destructured or rendered. Someone wired the last mile up to the door and never opened it. Fixed: a new pure helper module (`components/lyra/composer/best-posting-time.ts`, 9 tests) computing the top-scoring slot per platform and its next real calendar occurrence, wired into a visible "Best time for {platform}: {day} {time} (based on N of your posts)" hint in the Composer with a one-click "Use this time" button that sets the schedule picker directly. Full suite (294 tests), typecheck, and build all clean; deployed and confirmed live.
