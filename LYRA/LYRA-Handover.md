...
## 🔴 ACTION REQUIRED — bulk import needs one manual check (added 2026-08-12)

**Bulk scheduling / CSV import (Wishlist item 5) is built, tested, and pushed** (`30cc996`…`2b2d023` on `main`). No migration and no cron needed — it adds no schema and no scheduled job. Netlify hosted CI and Railway auto-deploy from the push.

**The one thing left is a manual smoke test**, which no test can cover. On a workspace with at least one connected account:

1. Open **Calendar → Bulk import** (button sits next to "New post").
2. **Download template** — confirm the `.xlsx` opens with a working Platform dropdown listing *only* that workspace's connected platforms.
3. Fill in three rows deliberately: one fully valid, one with a bad platform value typed over the dropdown, one with a dead Media URL.
4. Upload it — confirm the review table shows the expected **ready / warning / blocked** split, and that the blocked row cannot be ticked.
5. Click Import — confirm the posts land on the calendar in the right status (`SCHEDULED`, or `PENDING_APPROVAL` if that workspace has client approval on), and that the valid row's media was re-hosted to an `s3.…amazonaws.com` URL rather than the original link.

**Judgement call worth confirming:** a row dated in the **past** is treated as a blocking error, not a warning. The spec asked for a "future-or-present" `scheduledAt` but nothing enforced it — and since the publish cron picks up any `SCHEDULED` post whose time has passed, importing last month's plan would otherwise fire every one of those posts the instant you click Import. If you'd rather it be a dismissible warning, that's a one-line change in `services/posts/bulk-import.ts`.
...
- **The overdue badge is derived at render time**, not read from the stored `slaAlertedAt` flag — computed in `GET /api/posts` from the same pure helper the cron uses, so the badge appears even before the cron has run, clears the moment a post is approved, and cannot disagree with the alert.
