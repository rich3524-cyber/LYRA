...
*Build approach (corrected 11 Aug 2026 — see the scope doc for full detail):* Both platforms still work via a webhook URL the user pastes into LYRA settings, but **the two mechanisms are no longer the same**. Microsoft fully retired Teams' classic "Incoming Webhook" connector in May 2026 — Teams now requires the user to set up a Power Automate Workflow (not a simple connector) to generate the URL, and messages post without LYRA branding (no custom bot name/icon on that platform). Slack's path is still a straightforward Incoming Webhook, just via a Slack App the user creates (legacy standalone webhooks are deprecated) rather than the old zero-app-registration flow. Neither platform needs LYRA-side OAuth for v1. A native Slack App with OAuth (Option B) is a follow-up if there is user demand.
