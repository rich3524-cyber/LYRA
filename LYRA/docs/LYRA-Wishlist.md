...
**5.** 📄 **Bulk scheduling / CSV import** — *spec + plan written, not yet built*
Agencies plan content in spreadsheets. Every post currently has to be created individually in the composer. A locked, downloadable **`.xlsx`** template (not a flexible "upload your own spreadsheet" importer, and not plain CSV despite the item's name) — columns: Date, Time, Platform (dropdown, scoped to the workspace's actually-connected accounts), Caption, Media URL (optional, LYRA fetches and re-hosts it). One row per platform. A review screen shows every row's status (ready/warning/error) before anything touches the calendar. No schema changes — reuses the exact approval-routing and media-compatibility logic every other post-creation path already has. Design spec: `lyra/docs/superpowers/specs/2026-08-12-bulk-scheduling-csv-import-design.md`. Implementation plan (5 TDD tasks): `lyra/docs/superpowers/plans/2026-08-12-bulk-scheduling-csv-import.md`. Next step: choose Subagent-Driven or Inline execution and build it.
