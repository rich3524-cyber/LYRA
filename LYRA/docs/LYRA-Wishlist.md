...
*Original full-portal scope (demoted, not deleted):* Agencies need a stripped-down client-facing view where clients can see their content calendar, approve or reject pending posts, and read AI draft responses — without accessing the full LYRA dashboard.
...
**4.** ✅ **Best time to post**
`PostMetrics` (likes, comments, shares, reach) is collected for every published post. The engagement heat map on the Brand page (`components/lyra/brand/engagement-insights.tsx`) surfaces it visually. `services/ai/engagement-analyzer.ts` turns each workspace's own history into per-platform `PostingPatterns` (needs 12+ published posts on a platform before it produces anything) — already used to bias the AI Schedule Generator's chosen times (`services/ai/schedule-generator.ts`), and, as of 8 Aug 2026, also surfaced directly in the single-post Composer: a "Best time for {platform}: {day} {time} (based on N of your posts)" hint with a one-click "Use this time" button. That last piece had actually been wired all the way down to `PostComposer` as a prop already (`postingPatterns`, typed `any`) but silently dropped — never destructured or rendered. Found while reviewing this wishlist item, fixed same day.

**5.** ✅ **Bulk scheduling / CSV import** — *built and pushed 12 Aug 2026; one manual smoke test outstanding*
Agencies plan content in spreadsheets. Every post previously had to be created individually in the composer. Now: a locked, downloadable **`.xlsx`** template (not a flexible "upload your own spreadsheet" importer, and not plain CSV despite the item's name) — columns: Date, Time, Platform (dropdown, scoped to the workspace's actually-connected accounts), Caption, Media URL (optional, LYRA fetches and re-hosts it to S3). One row per platform. A review screen shows every row's status (ready/warning/blocked) before anything touches the calendar. No schema changes and no new cron — reuses the exact approval-routing and media-compatibility logic every other post-creation path already has, so a batch import cannot bypass client approval. Entry point: Calendar → Bulk import, gated on `canWrite`. Design spec: `lyra/docs/superpowers/specs/2026-08-12-bulk-scheduling-csv-import-design.md`. Commits `30cc996`…`2b2d023`.

*Decided during the build:* a **past-dated row is a blocking error**, not a warning — the publish cron picks up any `SCHEDULED` post whose time has passed, so importing last month's plan would otherwise fire every one of those posts on Import. Flagged for Richard to confirm or downgrade.

*Known limitation:* a workspace with **two accounts on the same platform** (two Facebook Pages) can't disambiguate — the dropdown lists platform names, not account names, so the first-listed account wins. Deterministic and tested, but a real gap for multi-client agencies. Fixing it properly means putting account names in the dropdown, which changes the template format.
...
**8.** 📄 **Slack / Microsoft Teams notifications** — full scope doc: `Scope Docs - Future projects/LYRA-Slack-Teams-Scope.docx`
An enhancement to the notification layer (item 1) specifically for agency teams. Email alerts go to one person's inbox — a Slack or Teams message in a shared channel (e.g. `#client-acme-alerts`) means the agency owner, account manager, and social media manager all see a crisis or approval request simultaneously, without anyone having to forward anything. The team mobilises in parallel, not sequentially. Particularly valuable for after-hours coverage: an autonomous crisis firing at 11pm reaches whoever is on Slack, not just whoever checks email first.

*Status (11 Aug 2026): ✅ **Slack built, deployed, and confirmed live** — migration applied, hourly SLA cron running, and a real "Send test" confirmed messages post under the LYRA name, not Zernio's fallback branding. Microsoft Teams still outstanding.* Implementation spec: `lyra/docs/superpowers/specs/2026-08-11-slack-notifications-design.md`.

*Build approach — superseded twice in one day, final version:* The scope doc (and an earlier correction here) assumed both platforms would use a webhook URL pasted into LYRA settings. That was right for Teams and **wrong for Slack**: Zernio, already LYRA's unified social API, added Slack as a supported platform. Slack now connects via **one-click Zernio OAuth** from Settings — no Slack App for the customer to create, no webhook URL to store, and Zernio owns the OAuth and token rotation. This delivers the "native Slack App with OAuth" idea that was previously parked as an optional v2 follow-up. Trade-offs accepted knowingly: Zernio bills per connected account, there is no Block Kit (the deep link is a labelled mrkdwn link, not a button), and delivery now depends on Zernio's uptime — mitigated by email remaining the baseline path for crisis alerts. The Slack channel is stored as its own `NotificationChannel`, deliberately **not** a `SocialAccount`, so an alerts channel never appears in the composer, calendar, analytics, or Inbox.

*Teams remains as the scope doc describes it:* Microsoft fully retired the classic "Incoming Webhook" connector in May 2026, so Teams needs a Power Automate Workflow, Adaptive Card payloads to keep interactive elements, and cannot carry LYRA branding. Zernio does not support Teams, so that platform keeps the webhook design.

*Also built alongside it:* **approval SLA tracking**, which did not previously exist anywhere in LYRA — a per-workspace deadline (default 4h before a post's scheduled time, with a 24h-since-submission fallback for posts with no scheduled time), an hourly cron that alerts once per submission, and an "Approval overdue" badge in the calendar and post detail panel. This also covers most of the *SLA tracking on approvals* idea listed under the Agency operations layer below, except the client reminder and optional auto-approve.

*Plan gating as built:* Agency plan, **or** Pro with the Crisis Aware add-on — the latter gets the full event set, not crisis alerts alone.
...
*Last updated: 24 Aug 2026 — pointed items 2, 3, 6, and 14 at the new Metricool gap-closure roadmap (`lyra/docs/superpowers/specs/2026-08-24-metricool-gap-roadmap-design.md`); item 2 (Client portal) rescoped from a full portal UI to an email-link approval flow, full-portal scope demoted to a later upsell rather than built now.*
*Previous update (11 Aug 2026, evening) — asked which items to tackle next; recommended (and Richard picked) item 2 (Client portal), item 3 (Team invitations), and item 6 (Email digest) as the highest-leverage picks, with item 5 (CSV import) as an honorable mention Richard also wanted to move on. Item 5 fully designed and planned this session — spec + a 5-task TDD implementation plan, not yet built (see item 5's own note for paths). Item 6 brainstorming had just begun, no design decisions made yet, when the session paused for a laptop switch.*
*Previous update (11 Aug 2026) — item 8, Slack/Microsoft Teams notifications, given a full scope doc (`Scope Docs - Future projects/LYRA-Slack-Teams-Scope.docx`). Along the way, corrected a real inaccuracy in the original note: Microsoft fully retired Teams' classic Incoming Webhook connector in May 2026, so "the same webhook mechanism works for both platforms" is no longer true — Teams now needs a Power Automate Workflow instead, with no LYRA branding on messages (a real, confirmed platform limitation, not a build gap). Slack's path is still simple but now requires a Slack App rather than a zero-registration webhook, since standalone legacy webhooks are deprecated there too.*
*Previous update (8 Aug 2026, later) — item 4, Best time to post, marked ✅: found the Composer half was already fully wired up to real data (`postingPatterns` threaded down as a prop) but silently unused — the prop was declared and passed but never destructured or rendered in `PostComposer`. Finished it: a "Best time for {platform}" hint with a one-click apply button, using the same workspace-specific data that already informs the AI Schedule Generator.*
*Previous update (8 Aug 2026) — added item 18, SOC 2 / ISO 27001 certification, to the Compliance section (renamed from "Needed Before Scaling to Europe" since this item's trigger is enterprise procurement, not geography) — parked as not currently needed, with cost/trigger notes for when it's worth revisiting.*
