...
- **Workers** (`workers/`) — 5 BullMQ workers (post publisher, comment monitor, AI responder, brand sync, competitor monitor) running as a single long-lived process (`workers/index.ts`) on Railway. Deployed by the `deploy-workers` job in `.github/workflows/deploy.yml`, which runs `railway up` after the app build succeeds.
...
- **Workers:** push to `main` → after the app build passes, `.github/workflows/deploy.yml`'s `deploy-workers` job runs `railway up --service lyra-workers --detach`. Railway builds via its Nixpacks builder using `railway.toml`'s `buildCommand`/`startCommand` — the repo's `Dockerfile.worker` is **not** currently wired into the deploy and should not be assumed to reflect the real worker environment.
