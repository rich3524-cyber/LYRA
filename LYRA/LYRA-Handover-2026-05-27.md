...
**Date:** May 2026 (updated 27 May 2026)  
...
### 27 May 2026 — Session 30 (UX Sprint — Showcase Prep)

Three UX features shipped ahead of the Friday 29 May showcase. Commit `fcb6d33`.

#### Command-centre workspace dashboard (`app/(dashboard)/workspace/[workspaceId]/page.tsx`)

The workspace overview page was rewritten as an intelligence command centre:

- **Setup progress card** — shown until 100% complete. Gradient progress bar (blue → teal → green) above a 4-step milestone checklist. Each incomplete milestone is a direct link to the relevant page (Settings → connect accounts, Brand → build profile, Compose → schedule first post, Settings → activate AI). Disappears automatically when all steps are done.
- **Publishing today timeline** — horizontal scrollable strip of post chips, each showing scheduled time, platform code, and content preview. Appears only when posts are scheduled for today.
- **Enriched stat cards** — the three stat cards (Scheduled this week / Pending responses / Connected accounts) now carry a contextual sub-text line: "Inbox clear", "3 posts ahead", "2 platforms active", etc.
- **Urgent comment panel** — when the inbox has pending items, a second column appears alongside Recent Posts. Shows the most recent pending comment preview and a "Review and respond" CTA button linking to the inbox.

The page now fetches: today's scheduled posts, the most recent pending comment, and setup-progress fields on the workspace (aiResponseMode, brandProfile, _count).

---

#### LYRA Assistant visual lift (`components/lyra/assistant/assistant-report-view.tsx`)

The Assistant page was redesigned with a premium split-panel layout:

- **Sticky left sidebar (desktop):** LYRA Assistant wordmark with purple Sparkles icon, quarter selector dropdown, Generate / Regenerate button (purple-accented, matching the sidebar nav), Export PDF button, and mini stat readout (total posts, avg engagement, best platform, top theme) once a report exists.
- **Instrument Serif headings:** the "Quarterly Review" and "Next 3 Months" section headings now use `font-display text-3xl` (Instrument Serif 400), giving them the same weight as page-level display titles.
- **Ruled divider** between the Quarterly Review and Next 3 Months sections (`border-t border-background-border`).
- **Month card headings** use `font-display text-xl` for visual hierarchy within the strategy section.
- **Mobile-responsive:** on screens narrower than `lg`, the left panel collapses entirely and controls move into a header row (quarter dropdown + export icon + generate button), maintaining full functionality.

---

### 27 May 2026 — Session 29 (LYRA Assistant + Deploy Fixes)

#### LYRA Assistant — new feature (commit `25b6afd`)

Full quarterly report + 3-month content strategy feature. Gated to PRO and AGENCY plans; Starter sees an upsell screen.

**How it works:**
- User navigates to LYRA Assistant in the sidebar
- Clicks "Generate Report" — the backend analyses the last 90 days of posts, engagement, and competitor data using Claude
- Returns a structured `ReportData` object: period, platform performance, insight narrative, by-platform breakdown, and a 3-month strategy with monthly content pillars, key dates, and post-frequency recommendations
- Report is stored as an `AssistantReport` DB record and displayed in the client component
- "Export PDF" calls a separate route that generates a PDF via `@react-pdf/renderer` and returns a signed S3 URL

**New files:**
- `services/assistant/report-types.ts` — TypeScript types for `ReportData`, `PlatformPattern`, `StrategyMonth`, etc.
- `services/assistant/report-generator.ts` — `generateAssistantReport(workspaceId)`: queries DB data, calls Claude claude-sonnet-4-6, parses and structures the response
- `services/assistant/pdf-generator.ts` — `generateReportPdf(report)`: renders a branded PDF using `@react-pdf/renderer`
- `app/api/assistant/generate/route.ts` — `POST` — triggers report generation, stores `AssistantReport` in DB, returns the new report
- `app/api/assistant/reports/route.ts` — `GET` — lists existing reports for a workspace
- `app/api/assistant/[reportId]/export-pdf/route.ts` — `GET` — generates PDF, uploads to S3, returns a signed download URL
- `app/(dashboard)/workspace/[workspaceId]/assistant/page.tsx` — server page: plan gate (STARTER → upsell, else → AssistantReportView)
- `components/lyra/assistant/assistant-report-view.tsx` — main client component (redesigned in Session 30)
- `components/lyra/assistant/assistant-upsell.tsx` — upsell page for Starter users (blurred preview, Pro/Agency plan cards, upgrade CTA)

**Sidebar change:**
- LYRA Assistant nav item added to `components/lyra/app-shell/sidebar.tsx` below SEO, separated by a `pt-3` top padding divider
- Styled with `border border-purple-500/30` at rest, `bg-purple-500/10 border-purple-500/70 text-purple-300` when active
- `overflow-hidden` on `motion.aside` changed to `overflow-x-hidden` — this was preventing the LYRA Assistant nav item from appearing in the sidebar on production

**Documentation updated (same commit):**
- `docs/legal/LYRA-Instruction-Manual.md` — v1.1, added Section 10 (LYRA Assistant), renumbered Pricing/Billing/Privacy sections, updated Pro tier inclusions
- Scope document updated to v1.4 with Build Status section
- Implementation plan updated to v1.4 with correct deployment config and phase status markers

---

#### Netlify base path fix (commit `25b6afd`)

The `netlify.toml` had `base = "LYRA/lyra"` set, which was telling Netlify to deploy from the outer OneDrive-level git repo's `LYRA/lyra/` subdirectory. This caused Netlify to use a stale checkout and deploy old code regardless of what was pushed. Fixed by removing the `base` directive entirely — Netlify now deploys from the inner repo root (the correct repo at `LYRA/lyra/`).
...
The `DIRECT_URL="$DATABASE_URL"` override that was in the build command in earlier sessions has also been removed. The `DATABASE_URL` env var (Supabase PgBouncer, port 6543, `?pgbouncer=true`) is used for the app, and `DIRECT_URL` (Supabase direct, port 5432) is used by Prisma for migrations. Both are now set correctly in Netlify environment variables.
...
- **Build command:** `npx prisma generate && npx prisma db push --accept-data-loss && npm run build`
- **Publish directory:** `.next`
- **Node version:** 20
- **Plugin:** `@netlify/plugin-nextjs`

The build runs `prisma db push` automatically on every deploy — schema changes are applied to the production database as part of the build. No manual schema step is needed after deploys.

**Important — git repo structure:** The active git repo is the inner repo at `LYRA/lyra/`. All commits and pushes happen from there. The `netlify.toml` has no `base` directive — Netlify deploys from the repo root (which is `LYRA/lyra/`). A stale `base = "LYRA/lyra"` setting caused Netlify to try to deploy from an outer OneDrive-level repo in earlier sessions; this was fixed in commit `25b6afd`.
...
| `/api/assistant/generate` | POST | Generate quarterly report + 3-month strategy via Claude |
| `/api/assistant/reports` | GET | List existing assistant reports for a workspace |
| `/api/assistant/[reportId]/export-pdf` | GET | Generate PDF, upload to S3, return signed URL |
...
| `lyra/netlify.toml` | Build config — `prisma db push` for automatic schema sync; no `base` directive |
| `lyra/services/assistant/report-types.ts` | TypeScript types for `ReportData`, `PlatformPattern`, `StrategyMonth` |
| `lyra/services/assistant/report-generator.ts` | `generateAssistantReport(workspaceId)` — queries DB, calls Claude, returns structured report |
| `lyra/services/assistant/pdf-generator.ts` | `generateReportPdf(report)` — renders branded PDF via `@react-pdf/renderer` |
| `lyra/app/api/assistant/generate/route.ts` | POST — triggers report generation, stores `AssistantReport` in DB |
| `lyra/app/api/assistant/reports/route.ts` | GET — lists existing assistant reports for a workspace |
| `lyra/app/api/assistant/[reportId]/export-pdf/route.ts` | GET — generates PDF, uploads to S3, returns signed download URL |
| `lyra/app/(dashboard)/workspace/[workspaceId]/assistant/page.tsx` | Server page — plan gate: Starter → upsell, Pro/Agency → AssistantReportView |
| `lyra/components/lyra/assistant/assistant-report-view.tsx` | Split-panel report client component (sticky left controls, scrollable right content) |
| `lyra/components/lyra/assistant/assistant-upsell.tsx` | Upsell page for Starter users |
| `lyra/lib/setup-progress.ts` | `computeSetupProgress(workspace)` — derives percent + 4 milestone booleans |
| `lyra/components/lyra/app-shell/setup-progress.tsx` | Sidebar setup progress widget (collapsed ring / expanded gradient bar + milestones) |
...
1. **Inbox UI** — comments API exists (`/api/comments`), inbox page at `/workspace/[id]/inbox` needs a UI. Workers are live and populating `Comment` rows.
...
3. **Crisis Aware UI** — the data model and detection logic exist; build the workspace settings toggle and the in-app crisis alert/resolve UI.
